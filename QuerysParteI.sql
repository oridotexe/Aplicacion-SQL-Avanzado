...
SELECT V.VENDEDOR, CN.CANAL_VENTA, F.ID_FACTURA, F.TOTAL_FACTURA, COALESCE(COB.VALOR_COBRADO, 0), 
    ROUND((COALESCE(COB.VALOR_COBRADO, 0)/F.TOTAL_FACTURA)*100 , 2) AS Porcentaje, 
    CASE 
        WHEN (COALESCE(COB.VALOR_COBRADO, 0) / F.TOTAL_FACTURA) = 1 THEN 'Cobro Total'
        ELSE 'Cobro Parcial'
